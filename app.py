...
This unified app.py integrates the environment logic directly with the UI 
for optimal resource usage on free-tier Spaces.
"""
...
from models import SupportAction, PublicSupportState
...
# Global state
# In-memory storage for active environments and session metrics
...
def _cleanup_sessions():
    ...
    if len(active_sessions) > MAX_SESSIONS:
        # Remove oldest 10 sessions (approximate by sorting keys if they are UUIDs/timestamps)
        # For simplicity, just clear all if it gets too large
...
def env_reset(difficulty: str, seed: Optional[int] = None) -> tuple:
    ...
    try:
        # If seed is 0 or negative, use None for real randomness
